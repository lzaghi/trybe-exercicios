...
os número que a outra irá escolher. Cada jogador escolhe 5 números
=de 0 a 10, inclusive. A pontuação final é calculada da seguinte forma:
...
def nota_partida(escolhas, adivinhas):
    return max(escolhas.difference(adivinhas))  # O(n)


def nota_redutor(escolhas, adivinhas):
    return min(escolhas.difference(adivinhas))  # O(n)


def nota_individual(escolhas, adivinhas):  # O(n)
...
def quem_ganhou_blefe(jogadas):
    jogadores = list(jogadas.keys())  # O(n)
